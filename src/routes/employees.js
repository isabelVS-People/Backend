...
router.patch('/:employeeId', requireRole('lider', 'admin_rrhh'), requireSameTeam, async (req, res, next) => {
  ...
  try {
    ...
    if (newLevel < 1 || newLevel > 5) { await client.query('ROLLBACK'); return res.status(400).json({ error: 'Nivel entre 1 y 5' }); }
    ...
    const updated = await pool.query(
      `SELECT e.*, r.name AS role_name, rf.name AS family_name
       ...
       LEFT JOIN role_families rf ON r.family_id = rf.id WHERE e.id = $1`,
      [employeeId]
    );
...
router.get('/:employeeId/history', authenticate, async (req, res, next) => {
  try {
    ...
    const result = await pool.query(
      `SELECT ch.id, ch.change_date, pr.name AS previous_role, nr.name AS new_role,
        ch.previous_level, ch.new_level, u.name AS changed_by
...
router.put('/:employeeId/requirements', requireRole('admin_rrhh'), requireSameTeam, async (req, res, next) => {
  ...
  try {
    ...
    if (!Array.isArray(values)) return res.status(400).json({ error: 'Se espera un array' });
    await client.query('BEGIN');
    for (const v of values) {
      await client.query(
        `INSERT INTO additional_requirement_values(employee_id, requirement_def_id, value, country)
         VALUES($1,$2,$3,$4) ON CONFLICT(employee_id, requirement_def_id)
