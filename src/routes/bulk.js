...
router.use((req, res, next) => {
  req.user = { userId: 1, role: 'admin_rrhh', country: 'Argentina' };
  next();
});
...
// Países válidos en minúsculas para comparación flexible
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          // Validación de país — case insensitive y acepta "Peru" sin tilde
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const fam = await client.query(
            `INSERT INTO role_families (name) VALUES ($1)
